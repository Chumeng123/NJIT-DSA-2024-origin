...
public class Algorithms {
    ...
    public static <T> void sortWithComparator(T[] array, Comparator<T> comparator) {
        if(array != null && array.length > 0){
            quickSortWithComparator(array, comparator, 0, array.length - 1);
        }
    }

    private static <T> void quickSortWithComparator(T[] array, Comparator<T> comparator, int start, int end){
        if(start < end){
            int pivotIndex = partitionComparator(array, comparator, start, end);
            quickSortWithComparator(array, comparator, start, pivotIndex - 1);
            quickSortWithComparator(array, comparator, pivotIndex + 1, end);
        }
    }

    private static <T> int partitionComparator(T[] array, Comparator<T> comparator, int start, int end) {
        return start;
    }
